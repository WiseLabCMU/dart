...
def awr1843boost_az8(
    ...
    """Compute 8 antenna gains for the 8 azimuth bins of the TI AWR1843BOOST."""
    gain = awr1843boost(theta, phi)

    w = jnp.sin(theta) * jnp.pi
    n = jnp.arange(8)

    def column(b):
        bin = (b / 8 * 2 - 1) * jnp.pi
        return gain * jnp.abs(jnp.sum(jnp.exp(jnp.outer(-1j * n, w - bin)), 0)) / 8

    return jax.vmap(column)(n)
